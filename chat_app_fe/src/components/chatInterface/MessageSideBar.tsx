...
const MessageSideBar: React.FC<Props> = ({ conversations, isOpen, toggleSidebar, onConversationClick }) => {
    return (
        <div 
            className={`
        ...
        >
            <div className="flex justify-between items-center mb-4">
                <h2 className={`text-xl font-semibold ${!isOpen && 'sr-only'}`}>Conversations</h2>
                ...
                {conversations.map(conversation => (
                    isOpen ? (
                        ...
                        <div
                            ...
                            onClick={() => onConversationClick(conversation as Conversation)}
                        >
                            <h3 className="text-lg font-semibold truncate">{conversation.name}</h3>
                        </div>
                    )
                ))}
            </div>
